...
The intruder will proceed as follows:
\begin{itemize}
\item{Look up $q$ for each original record in the synthetic if no records are found score 0.}
\item{For those found (in synthetic original, $iSO$ as a proportion of original records), check if all such records have the same level of the target $t$ in the synthetic data as in the original. Score 1 for each such original record where this apparent disclosure corresponds to the value of $t$ in the original data;
Otherwise score 0. The mean of this score is $DiSCO$, disclosive in synthetic correct in original.}
\end{itemize}
...
As we move from $DOS$ to $DiSCO$ and then $DisDiO$ we can see how the \%disclosive is affected by different conditions. Here lack of $q$ levels in the synthetic data reduces disclosure to 64\%; then the requirement for correc6t attribution redices it to around 9\%; finally restricing to 
records disclosive in the original reduces it to 6\%,

The $DiO$ and $DiSCO$ measures are not restricted to disclosures that are identified from unique records for $q$ in either the original or the synthetic data. The number of records contributing to each disclosive $qt$ cell in the synthetic table is the denominator that applies to that record. The columns \texttt{max denom} and \texttt{mean denom} refer to the denominators for the records disclosive in the synthei(c data that contribute to the $DiSCO$ or $DISCDO$ measures. We can see from the mean that here the majority of disclosive records had unique key combinations in the synthetic data, and the maxima was 4, 5 or 6 for different syntheses. Large denominators can be an indication that some of the disclosure may be coming from strong relationships between variables in the data that might even be expected a-priori. This aspect is discussed further in Section \ref{sec:onetwoway}. The disclosure measures can be restricted to those with small denominators by using
...
have increased and also that $DiO$ is now the same for all targets and equal to the $UiO$ value before the denominator exclusion. This makes sense because 
...
@
\renewcommand{\baselinestretch}{1.5}
We cn see that it was the category "NO" of \texttt{workab} that contributed most to the
disclosure risk; most survey respondents (93\%) had never worked abroad. This level of
the target accounted for 89\% of apparent disclosures for \texttt{workab}. To
...
\begin{equation}
 DiO = 100\sum^q{\sum^t{(d_{tq} |pd_{tq} = 1})}/N_d.
\end{equation}

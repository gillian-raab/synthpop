...
Here we illustrate the basic use of \texttt{disclosure.summary}. If the parameter \texttt{targets} is not specified, all the variables in the synthetic data that are not part of keys are used as targets. Results are presented for one measure of identity disclosure and one measure of attribute disclosure.
The defaults for identity disclosure are \texttt{UiO} for original and \texttt{repU} for synthetic, and for attribute disclosure \texttt{DiO} for original and \texttt{DiSCO} for synthetic. These and other measures will be explained in Sections \ref{subsec:ident} and \ref{subsec:attrib}.
...
Now we summarise the disclosure measures from a set of 4 keys, with three other variables as targets. 
<<discsumm, size="tiny">>=
...
in the plot. The columns \texttt{check1} and \texttt{check2} in the table are blank because none of these variables
...
The measure ''DiO" (disclosive in the original) tells us that these 4 keys would identify a unique value of each of these targets for over 50\% of the original records.  $DiSCO$ (Disclosive in Synthetic Unique in Original) is the proportion of the original records that are disclosive in the original and also in the synthetic data with a correct attribution to the target. 
...
One of the outputs of the function \texttt{disclosure} is \texttt{ident}, a table of identity disclosure measures as illustrated by this example, using the same target and keys as the example in Section \ref{sec:simpexamp} but now calculated for a synthetic object with 5 data sets. We first create \texttt{t5} an object of class \texttt{disclosure} and then print out the identity disclosure measures for each synthetic data set.
\renewcommand{\baselinestretch}{1.0}

<<mof5, size="small">>=
s5 <- syn(ods, seed = 8564, m = 5, print.flag = FALSE)
t5 <- disclosure( s5, ods, keys =  c("sex", "age", "region",
     "placesize"), target = "edu", print.flag = FALSE)
...
These equations have defined one attribute disclosure measure for the original data, $DiO$ and three for the synthetic data, $KiSiO$, $DiSCO$ and $DiSCO\_UiO$. This code calculates and prints them from  \texttt{disclosure{}} for each of five synthetic data sets:
\renewcommand{\baselinestretch}{1.0}
<<>>=
